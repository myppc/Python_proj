...
class every_day_tactics(base_tactics):
    ...
    def today_decision(self):
        ...
        if self.hold_stock ==0 : 
            if cur_price > max_20:
                print(self.today,"BUY","突破二十日最大值,当前价格",cur_price,"20日最高价",max_20,"买入",round(self.last_money  * 0.25,2),"(Y/N)?")
                if input() == "y":
            ...
            if (avg10 - cur_price)/avg10 > 0.05 and (cur_time_stamp - last_time_stamp) /(24*3600) > 5:
                ...
                print(self.today,"BUY","补仓,当前价格",cur_price,"10日均价",avg10,"买入",round(self.cur_money  * 0.25,2),"(Y/N)?")
                if input() == "y":
            ...
            if (cur_price - hold_price)/hold_price * 100 > self.sell_per + 10 and (cur_time_stamp - last_time_stamp) /(24*3600) > 5:
                self.sell_per += 10 
                print(self.today,"SELL","触发卖出点",cur_price,"持仓价",hold_price,"卖出",round(self.hold_stock  * 0.2,2),"(Y/N)?")
                if input() == "y":
                    self.sell(self.hold_stock * 0.2 ,"卖出20%")
                return;
            if (cur_price - hold_price)/hold_price * 100 > 20 :
                self.sell_per = 100
                print(self.today,"SELL","触发止盈点",cur_price,"持仓价",hold_price,"卖出",round(self.hold_stock,2),"(Y/N)?")
                if input() == "y":
